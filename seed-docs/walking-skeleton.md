...
完了条件: 全 gate が「skip ではなく実行されて緑」。1 spec でも未検証の skip を残したまま「一周した」と宣言しない。deploy する PJ は promote 直後の smoke 3 項目 (seed-docs/screen-loop.md ⑩) まで含めて完了とする。ここまで緑になって初めて画面量産 (seed-docs/screen-loop.md) に入る。
