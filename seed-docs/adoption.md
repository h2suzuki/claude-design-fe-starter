...
**gate を包む runner を自作するなら、両方の rc を通す。** `bun run --cwd pp gate` は `test` と `require-no-skips` の 2 本で、後者だけが「未検証の skip が残っていないか」を見る。画面ごとに回す runner を書いて前者の rc だけ返すと、**skip 検査が落ちても runner は緑を返す**。適用先はこれを踏み、緑を疑う検証を別に立てるまで気づけなかった（2026-08-30）。gate 自体は正しく赤を出していたので、**壊れるのは gate ではなく、その外側で数える側**である。
