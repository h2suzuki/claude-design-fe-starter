...
function reviewOf(reviewDir, slug) {
  ...
  return {
    ...
    notes: findings.filter((finding) => finding?.kind === 'note').length,
  };
}
...
function reviewCell(review) {
  return review ? `${review.model ?? '—'}/${review.effort ?? '—'}・指摘 ${review.findings}（気づき ${review.notes ?? 0}）・open ${review.open}` : 'なし';
}
