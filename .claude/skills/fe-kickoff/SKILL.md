...
5. 検証条件を確定する: `pp/src/config.ts` の基準 viewport 2 点・locale/timezone・固定時刻・app の mount 点。画面ごとの route・セレクタ・操作は `pp/src/screens.ts` へ、凍結した画面から順に登録する。描画前に決めるべき状態（テーマ・言語など、`seed-docs/pre-implementation-questions.md` の同名項）があれば `prePaintStates` として同じ登録点に書く
