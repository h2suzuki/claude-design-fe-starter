...
3. 検証条件を確定する: `pp/src/config.ts` の基準 viewport 2 点・locale/timezone・固定時刻
4. Claude Design の 2 project 体制を用意する（design-system 型 = 部品ライブラリ、通常 = mock 用）。環境変数 `DESIGN_PROJECT_ID` を設定する
5. 発注規約 1 枚を用意する（/design-order が `seed-docs/design-order-template.md` から組み立てる）
6. `{{...}}` placeholder を grep で列挙し、残る差し替え点を確定させる
7. walking skeleton を一周する: 最初の 1 部品で mock → /mock-freeze → 実装 → parity → sweep。pp の skip が全て外れて緑になってから画面量産に入る
