...
3. 各 screenshot について次を判定し、疑わしい点を指摘として書く:
   ...
   - **見本データの読み方**: 見本データの変化（枠数・会場・時間帯・曜日の違い、見本カレンダーと見本カードの値の差）は、デザイナーが FE に示唆を与えるために選んで入れたもの。整合性の欠陥として書かず、「この変化を FE が扱えるか」の検査項目（fixture の case 候補）として気づきに書く。文言の言い回し・揃え方の違いも気づき（mock:integrity の MOCK204 と同じ線）
4. 所見ごとに `kind` と処置を付ける。`kind` は `defect`（利用者が誤った行動を取る / 操作が意図と違う結果になる。例: 月を送っても前月の枠で決定される、エラー帯が埋めた項目も列挙する）か `note`（気づき: 見た目の好み・情報設計の提案・事実確認・見本データの変化から出る検査項目。promote は止めない）。処置は `fixed`（app か mock を直した。直した commit を text に添える）/ `keep-impl:<DESIGN-POLICY.md の entry 文字列>`（日付付き裁定として台帳に載せた）/ `open`（未裁定。defect の open が残ると review:check が赤のまま。note の open は残してよい）
