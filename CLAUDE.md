...
- 完成条件は 3 分類: 基準幅 = structural pixel 一致 / 中間幅（`SWEEP_WIDTHS` の全幅）= invariant / 状態 = 挙動一致。判定は `npm --prefix pp run gate` が行い、未検証の skip が 1 件でも残れば落ちる。対象部品が無い画面の gate だけ `pp/gate-not-applicable.json` に日付と理由を付けて宣言できる
<!-- fe-starter:end -->
