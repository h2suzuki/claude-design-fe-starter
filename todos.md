...
- [x] rebase を採るなら、先に §6 を変えてから文書どおり実行し、gate 緑まで確認する。merge のまま据え置くならその理由を §6 に書く — merge 据え置き。不採用の理由（作業 branch の全 commit hash が更新のたびに変わり、裁定・根拠・出典の hash 参照が失われる）を §6 に書いた。手順は使い捨て repo で 2 通り実測（常道は衝突なし・PJ 値残存・機構更新到達／add/add 衝突は 1 件で役割別に解決）。あわせて衝突解決の誤り（差し替え点でも main 側を採ると書いていた）を直した
