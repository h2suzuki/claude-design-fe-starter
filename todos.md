...
- [ ] 描画そのものが重い箇所（要素数の多いカレンダー・表・長いリスト）を実描画で測り、閾値を超えたものを挙げる
- [ ] 一周実証の完了後に着手する
...
資産の重さは `mock-lint` の MOCK104 で実装済み（`d579d79`。個別 1 MB 超を挙げ、合計も出す）。**描画そのものが重い箇所は静的には測れない** — H.S. 指摘（2026-08-28）の「PNG 4 枚やカレンダー」のうち、カレンダーは要素数と再描画のコストなので、実描画で計測する必要がある。閾値は環境で動くので、絶対時間でなく要素数など環境に依らない量を先に見る。
