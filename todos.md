...
- [ ] mock の script を読み、実装すると待ちが出る箇所を挙げる（外部 API 呼び出しを静的資産と区別する / 操作ハンドラの延長で呼ぶもの / 要素を量産するループ）
- [ ] 一周実証の完了後に着手する
...
資産の重さは `mock-lint` の MOCK104 で実装済み（`d579d79`。個別 1 MB 超を挙げ、合計も出す）。

残りは **mock の script を読む**側。H.S. 指摘（2026-08-28）「実測より、mock のコードから判別できるのでは？ SaaS の呼び出しをやっているところ。ボタンの延長などでやってたら、特に怪しい」。実描画での計測を当初案にしていたが、**操作の延長で走る処理は描画しただけでは出ない**ので、静的に読む方が確実。

適用先の凍結 mock を調べた範囲では、SaaS 呼び出しも要素量産のループも無い（`support.js` の fetch 3 件は runtime が React を unpkg から読む部分で、MOCK101 が既に挙げている）。**現物で確かめられる検出対象がまだ無い**ので、合成 fixture だけで作らず、実例が出てから着手する。
