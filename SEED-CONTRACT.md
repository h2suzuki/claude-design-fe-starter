...
| `.claude/hooks/` | `block-frozen-mock-edit.sh` `block-ui-before-mock.sh` `check-mock-baseline.sh` `suggest-pr-ci.sh` `block-land-without-ci.sh` | file 単位の add-only（exec bit は seed が記録した mode で配る） |
