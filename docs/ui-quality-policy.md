...
FE の発注・実装・検証・land 判定で守る品質規律を定める。FE 実装の発注書は、本ファイルと docs/ui-caveats.md（画面ジオメトリ安定原則・タブ毎状態・実装説明文の禁止）を必須参照に含めること。
...
- スクリーンショットの一次チェックは、画面判定の実績がある LLM へ委譲し、意味論的破綻（値の整合・表示の意味）を見る。壊れた画面を完了扱いした実績のある構成は使わない。
